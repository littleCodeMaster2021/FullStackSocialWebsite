...
describe('Test post event.', () => {
  ...
  beforeEach(inject((_$controller_, _$rootScope_, $httpBackend) => {
    ...
    httpBackend.when('GET', '/profile/Test@seas.upenn.edu/post').respond(
      {
        post: [
          {
    ...
    httpBackend.when('POST', '/addFollowers/yuhangu.m@gmail.com', {
    ...
    httpBackend.when('POST', '/editPostText/1571670583491', () => {
      // const jsonData = JSON.parse(postData);
      // expect(jsonData.Comment).toBe('hi');
      return true;
    }).respond(200, true);
  ...
  describe('post', () => {
    it('post event', async () => {
      ...
      expect($scope.visitUser('Test@seas.upenn.edu', 'Test@seas.upenn.edu')).toBe('/profile?Test@seas.upenn.edu');
      expect($scope.visitUser('Test@seas.upenn.edu', 'robin@seas.upenn.edu')).toBe('/otherprofilePost?Test@seas.upenn.edu?robin@seas.upenn.edu');
